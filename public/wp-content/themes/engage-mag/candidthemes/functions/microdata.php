 ...
 */
if (!function_exists('engage_mag_get_microdata')) :
    function engage_mag_get_microdata($context)
    {
        $data = false;

        if ('microdata' !== apply_filters('engage_mag_schema_type', 'microdata')) {
            return false;
        }
        ...
        if ('body' === $context) {
            ...
            $type = apply_filters('engage_mag_body_itemtype', $type);
        ...
        if ('article' === $context) {
            $type = apply_filters('engage_mag_article_itemtype', 'CreativeWork');
        ...
        if ($data) {
            return apply_filters("engage_mag_{$context}_microdata", $data);
        }
    }
endif;
 ...
 */
if (!function_exists('engage_mag_do_microdata')) :
    function engage_mag_do_microdata($context)
    {
        echo engage_mag_get_microdata($context); // WPCS: XSS ok, sanitization ok.
    }
endif;